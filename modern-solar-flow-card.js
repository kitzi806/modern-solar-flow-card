...
class ModernSolarFlowCard extends HTMLElement {
  ...
  _updateContent() {
    ...
    const styleVars = isDark ? `
      ...
      --ms-color-wp: #fb8c00;
      --ms-glow-green: drop-shadow(0 0 5px rgba(74, 222, 128, 0.8));
      --ms-glow-red: drop-shadow(0 0 5px rgba(248, 113, 113, 0.8));
      --ms-glow-blue: drop-shadow(0 0 5px rgba(96, 165, 250, 0.8));
      --ms-glow-orange: drop-shadow(0 0 5px rgba(251, 140, 0, 0.8));
    ` : `
      --ms-bg: linear-gradient(180deg, #e1f5fe 0%, #ffffff 100%);
      --ms-card-border: none;
      --ms-shadow: 0 4px 15px rgba(0,0,0,0.05);
      --ms-circle-bg: #ffffff;
      --ms-circle-border: 4px solid #e0e0e0;
      --ms-text-val: #333333;
    ...
    const css = `
        .solar-root {
          ...
          background: var(--ms-bg); border: var(--ms-card-border); border-radius: 20px;
          box-shadow: var(--ms-shadow); overflow: hidden; display: flex; flex-direction: column;
          font-family: 'Roboto', sans-serif; box-sizing: border-box; transition: height 0.3s ease;
        }
        .diagram-area { position: relative; flex-grow: 1; width: 100%; }
        .price-badge {
          position: absolute; top: 15px; right: 15px; background: var(--ms-circle-bg); opacity: 0.9;
          border: 1px solid var(--ms-text-unit); padding: 4px 10px; border-radius: 15px;
          font-size: 12px; font-weight: 700; color: var(--ms-text-val); z-index: 50; display: flex; align-items: center; gap: 6px;
        }
        ...
        .val { font-size: 20px; font-weight: 900; color: var(--ms-text-val); line-height: 1.1; z-index: 5; }
        .unit { font-size: 12px; font-weight: 500; color: var(--ms-text-unit); margin-left: 2px; }
        .sub-val { font-size: 13px; font-weight: 600; color: var(--ms-text-unit); margin-top: 2px; z-index: 5; }
        .label { font-size: 10px; font-weight: 700; color: var(--ms-text-label); text-transform: uppercase; margin-top: 2px; z-index: 5; }
        ...
        .path-flow { fill: none; stroke: var(--ms-color-solar); stroke-width: 6px; stroke-dasharray: 12; opacity: 0; transition: stroke 0.3s ease, opacity 0.3s ease; }
        
        .path-flow.flow-red { stroke: var(--ms-color-red) !important; filter: var(--ms-glow-red); }
        .path-flow.flow-green { stroke: var(--ms-color-solar) !important; filter: var(--ms-glow-green); }
        .path-flow.flow-blue { stroke: var(--ms-color-blue) !important; filter: var(--ms-glow-blue); }
        .path-flow.flow-wp { stroke: var(--ms-color-wp) !important; filter: var(--ms-glow-orange); }
    ...
    let wpStateText = 'AUS'; let isWpRunning = false;
    if (wpEntity) {
      ...
      isWpRunning = hasPower || isOn; if (config.invert_wp) isWpRunning = !isWpRunning;
      if (isWpRunning) { wpStateText = hasPower ? `${Math.round(Math.abs(n))} W` : 'EIN'; } else { wpStateText = 'AUS'; }
    }
